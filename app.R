...
factor_columns <- c('required_car_parking_space', 'repeated_guest', 'type_of_meal_plan', 
                    'market_segment_type', 'room_type_reserved', 'booking_status', 
                    'no_of_previous_cancellations')

df[factor_columns] <- lapply(df[factor_columns], as.factor)

# Select relevant variables and subset the data
      ...
      tabItem(tabName = "beranda",
              titlePanel(
                carousel(width = 12,
                         id="mycarousel",
                         carouselItem(
                           tags$img(src = "Image/Home_Page.png")
                         ),
                         carouselItem(
                           tags$img(src = "Image/Team_Member.png")
                         )
                )
              )
      ),
